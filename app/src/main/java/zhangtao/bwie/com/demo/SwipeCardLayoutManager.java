...
public class SwipeCardLayoutManager extends RecyclerView.LayoutManager{
    ...
    private ItemTouchHelper mitem;
    public SwipeCardLayoutManager(RecyclerView see_recy, ItemTouchHelper helper) {
        this.mrecy = see_recy;
        this.mitem = helper;
    }
    @Override
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (itemCount > CardConfig.DEFAULT_SHOW_ITEM) {
            for (int position = CardConfig.DEFAULT_SHOW_ITEM; position >= 0; position--) {
                ...
                // recyclerview 布局
                layoutDecoratedWithMargins(view, widthSpace / 2, heightSpace / 6,
...
//                    view.setScaleY(1 - position * CardConfig.DEFAULT_SCALE);
    ...
    private View.OnTouchListener mOnTouchListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            RecyclerView.ViewHolder childViewHolder = mrecy.getChildViewHolder(v);
            if (MotionEventCompat.getActionMasked(event) == MotionEvent.ACTION_DOWN) {
                mitem.startSwipe(childViewHolder);
            }
            return false;
        }
    };
